...
def _tag_as_release(tag: dict) -> dict:
    """Synthesize a GitHub-release-shaped dict from a git tag.

    This fork publishes Docker tags via ``docker-publish.sh`` and may not
    always create GitHub Releases. The update checker still needs a version
    stream, so we fall back to ``/repos/{repo}/tags`` and present each tag
    as a release-like object the rest of the pipeline already understands.
    """
    ...
    return {
        ...
        "name": name,
        "body": "",
        "html_url": f"https://github.com/{GITHUB_REPO}/releases/tag/{name}",
        "published_at": "",
        "assets": [],
    }


async def _list_release_candidates(client: httpx.AsyncClient) -> tuple[list[dict] | None, httpx.Response]:
    """GitHub Releases for ``GITHUB_REPO``, falling back to git tags.

    Returns ``(None, response)`` on rate-limit so the caller can back off.
    An empty list means the repo has neither releases nor tags yet.
    """
    ...
    if _is_github_rate_limit_response(tags_response):
        return None, tags_response
    if tags_response.status_code >= 400:
        return [], response
    tags = tags_response.json()
    if not isinstance(tags, list):
        return [], response
    return [_tag_as_release(tag) for tag in tags if tag.get("name")], tags_response


def _pick_release(releases: list[dict], include_beta: bool) -> dict | None:
    """First matching release/tag, skipping prereleases unless opted in."""
...
async def _origin_points_at_repo(git_path: str, git_config: list[str], app_dir, expected_repo: str) -> bool:
    ...
    `<owner>/<repo>` matching `expected_repo` (e.g. "Nicolas-Cilia/Backoffice-Printing"),
...
async def check_for_updates(
    ...
    try:
        async with httpx.AsyncClient() as client:
            releases, response = await _list_release_candidates(client)
            ...
            release_data = _pick_release(releases, include_beta)
...
async def _perform_update(target_ref: str):
    ...
    try:
        ...
        # legitimately `git@github.com:Nicolas-Cilia/Backoffice-Printing.git` and the user
    ...
    if _is_windows_installer_install():
        ...
        return {
            ...
            "message": (
                ...
                f"Download the latest installer from https://github.com/{GITHUB_REPO}/releases."
            ),
        }
