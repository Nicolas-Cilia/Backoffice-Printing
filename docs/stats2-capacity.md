...
**What-if over ask:** when the target exceeds what the packer can start, the print-plan response lists every **short part** (`short_parts[]`: packed vs needed) and a **minimum extra printers** estimate for models that printed that part (else eligible models). Extras scale ``printers × target / capacity_ceiling`` under the **current operator schedule** — not ``needed/packed`` from the over-ask pack (that understates part throughput vs the eligible fleet and inflates counts). Per-part extras are independent lower bounds — shared models (e.g. X1C on TOP+BOT) must not be summed blindly. `binding_print_part` remains the worst short part for packer priority.
