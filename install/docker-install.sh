...
#
# Usage:
#   Interactive:  curl -fsSL https://raw.githubusercontent.com/Nicolas-Cilia/Backoffice-Printing/main/install/docker-install.sh -o docker-install.sh && chmod +x docker-install.sh && ./docker-install.sh
#   Unattended:   ./docker-install.sh --path /opt/bambuddy --port 8484 --yes
#
# Options:
#   --path PATH        Installation directory (default: /opt/bambuddy)
#   --port PORT        Host port to expose (default: 8484; container stays on 8000)
...
show_help() {
    ...
    echo "  --port PORT        Host port to expose (default: 8484)"
    ...
    echo "    ./docker-install.sh --path /srv/bambuddy --port 8484 --tz America/New_York --yes"
    echo ""
...
download_compose_file() {
    ...
    if [[ "$BUILD_FROM_SOURCE" == "true" ]]; then
        ...
        curl -fsSL -o docker-compose.yml \
            https://raw.githubusercontent.com/Nicolas-Cilia/Backoffice-Printing/main/docker-compose.yml
    fi
...
# Host port (browser). Container listen port stays 8000.
HOST_PORT=$HOST_PORT
PORT=8000
...
gather_config() {
    ...
    # Host port published by Docker (container still listens on 8000)
    [[ -z "$HOST_PORT" ]] && prompt "Port to expose" "$DEFAULT_HOST_PORT" HOST_PORT
...
main() {
    ...
    echo -e "  ${BOLD}Access BamBuddy:${NC}  ${CYAN}http://localhost:$HOST_PORT${NC}"
    echo -e "                    ${CYAN}http://$ip_addr:$HOST_PORT${NC} (from other devices)"
    echo ""
