...
.PARAMETER Port
    Host port to expose. Default: 8484 (container still listens on 8000)
...
function Update-ComposeForDockerDesktop {
    ...
    $content = $content -replace '(?m)^(\s*)#(\s*)- "\$\{HOST_PORT:-8484\}:8000"', '$1$2- "$${HOST_PORT:-8484}:8000"'
    $content = $content -replace '(?m)^(\s*)#(\s*)- "\$\{PORT:-8000\}:8000"', '$1$2- "$${HOST_PORT:-8484}:8000"'
...
if (-not $Port)        { $Port = [int](Read-Default 'Port to expose' '8484') }
if (-not $TimeZone)    {
