...
#   docker pull ghcr.io/nicolas-cilia/backoffice-printing:daily
#
# Prerequisites:
#   1. Log in to ghcr.io:
...
IMAGE_NAME="nicolas-cilia/backoffice-printing"
...
_Changelog truncated — see the full [CHANGELOG.md](https://github.com/Nicolas-Cilia/Backoffice-Printing/blob/main/CHANGELOG.md) for the complete list._"
    fi

    # Build pull commands for the release body
    PULL_COMMANDS=""
    if [ "$PUSH_GHCR" = true ]; then
        PULL_COMMANDS="docker pull ghcr.io/nicolas-cilia/backoffice-printing:daily"
    fi
    if [ "$PUSH_DOCKERHUB" = true ]; then
        if [ -n "$PULL_COMMANDS" ]; then
            PULL_COMMANDS="${PULL_COMMANDS}
# or
docker pull nicolas-cilia/backoffice-printing:daily"
        else
            PULL_COMMANDS="docker pull nicolas-cilia/backoffice-printing:daily"
        fi
    fi
